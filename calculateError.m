  ...
  if(dim == 1)
    ...
    if(org_width > org_height)
      ...
      while(org_width != org_size)
        original = original(1:end-1, 1:end);
        
        [org_width, org_height] = size(original);
      ...
      while(org_height != org_size)
        original = original(1:end, 1:end-1);
    ...
    if(dec_size > org_size)
      ...
      while(dec_size != org_size)
        decompressed = decompressed(1:end-1, 1:end-1);
        
        [dec_size, dec_height] = size(decompressed);
      endwhile
    ...
    elseif(org_size > dec_size)
      ...
      # Retiro a cada vez 1 linha/coluna
      while(dec_size != org_size)
        original = original(1:end-1, 1:end-1);
        
        [org_size, org_height] = size(original);
        
      endwhile
    endif
    
    # Erros
    err = norm(double (original) - double (decompressed))/(norm(double (original)))
